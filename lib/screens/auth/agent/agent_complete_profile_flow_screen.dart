...
class AgentCompleteProfileFlowScreen extends StatefulWidget {
  const AgentCompleteProfileFlowScreen({super.key});

  @override
  State<AgentCompleteProfileFlowScreen> createState() =>
      _AgentCompleteProfileFlowScreenState();
}

class _AgentCompleteProfileFlowScreenState
    extends State<AgentCompleteProfileFlowScreen> {
  final GlobalKey<FormState> _basicInfoKey = GlobalKey<FormState>();
  final GlobalKey<FormState> _contactLocationKey = GlobalKey<FormState>();
  final GlobalKey<FormState> _professionalInfoKey = GlobalKey<FormState>();
  ...
  void didChangeDependencies() {
    ...
    final savedPath =
        context.read<UserProfileProvider>().localProfileImagePath;
  ...
  Future<void> _loadInitialData() async {
    try {
      final AgentProfileModel data = await ProfileService.getAgentProfile();
  ...
  bool _validateCurrentStep() {
    ...
    if (_currentStep == 1) {
      final isValid = _contactLocationKey.currentState?.validate() ?? false;
    ...
    if (_currentStep == 2) {
      return _professionalInfoKey.currentState?.validate() ?? false;
    }
  ...
  Future<void> _goNext() async {
    ...
    try {
      if (_currentStep == 0) {
        await ProfileService.updateAgentBasicInfo(
          firstName: firstNameController.text.trim(),
          lastName: lastNameController.text.trim(),
        );
      } else if (_currentStep == 1) {
        await ProfileService.updateAgentContactLocation(
        ...
        await ProfileService.updateAgentProfessionalInfo(
  ...
  void _handleFinish() {
    ...
    Navigator.pushAndRemoveUntil(
      context,
      MaterialPageRoute(
        builder: (_) => const AgentEntryScreen(),
      ),
      (route) => false,
    );
  }
  ...
  Widget _buildStepIndicator(bool isDarkMode) {
    ...
    const labels = ['Basic', 'Contact', 'Professional', 'Finish'];
  ...
  Widget _buildContactLocationStep(bool isDarkMode) {
    final iconColor = isDarkMode
        ? Colors.white.withOpacity(0.7)
        : Colors.black.withOpacity(0.7);

    return Form(
      key: _contactLocationKey,
      child: Column(
        children: [
          ProfilePhotoPicker(
            key: ValueKey(
  ...
  Widget _buildProfessionalStep(bool isDarkMode) {
    final iconColor = isDarkMode
        ? Colors.white.withOpacity(0.7)
        : Colors.black.withOpacity(0.7);

    return Form(
      key: _professionalInfoKey,
      child: ProfileSectionCard(
        isDarkMode: isDarkMode,
        ...
        child: Column(
          children: [
            ...
            CustomTextField(
              labelText: 'Skills *',
              hintText: 'Example: Flutter, UI Design, REST API',
    ...
    return Column(
      children: [
        ...
        summaryRow(
          icon: HugeIcons.strokeRoundedMapsSquare02,
          label: 'Postal code',
          value: postalCodeController.text.trim(),
        ),
        const SizedBox(height: 12),
        summaryRow(
          icon: HugeIcons.strokeRoundedNoteEdit,
        ...
        summaryRow(
          icon: HugeIcons.strokeRoundedBriefcase01,
          label: 'Skills',
          value: skillsController.text.trim(),
        ),
        const SizedBox(height: 12),
        summaryRow(
          icon: HugeIcons.strokeRoundedDollarCircle,
          label: 'Hourly rate',
          value: hourlyRateController.text.trim(),
        ),
      ],
    );
  }
  ...
  Widget _buildCurrentStep(bool isDarkMode) {
    switch (_currentStep) {
      ...
      case 1:
        return _buildContactLocationStep(isDarkMode);
      case 2:
        return _buildProfessionalStep(isDarkMode);
      default:
  ...
  String _stepSubtitle() {
    switch (_currentStep) {
      ...
      case 1:
        return 'Add your phone, photo, and location details.';
      case 2:
        return 'Help clients understand your skills and pricing.';
      default:
        return 'Make sure everything looks right before continuing.';
    }
  }
